...
* 인스턴스를 새로 생성하지 않고 최초 호출시에 만들어 두었던
* 인스턴스를 재활용하는 패턴
*
* = 장점
* - 메모리 낭비를 방지가능하다.
*
* 최초 한번 new로 객체를 생성하고 해당 객체를 이후에 사용하도록 공유(static)하여
* 메모리 낭비 문제를 방지하는 효과가 있다.
*
* - 접근제어 : 최초 만들어진 객체를 공유해 주지 않으면 서비스를 사용할 수 없도록 제한 할 수있다.
*
* 문제점
* 1. 의존성 문제
* 만약 수정요청이나 확장 요청시 싱글톤으로 생선된 인스턴스를 참조하는
* 모든 클래스 들을 수정해야 한다.
*
* 2. private 생성자때문에 상속이 어렵다.
* [ 상속을 통한 자식 클래스를 만들 수 없다 -> 다형성 적용이 어렵다 ]
*
* 3. 테스트가 어렵다.
* 테스트 마다 공유하고 있는 인스턴스의 상태를 매번 초기화해야 하기 때문이다.
